...
const AddressSection: React.FC<AddressSectionProps> = ({ enderecos, onEnderecosChange }) => {
  ...
  const handleAddressChange = (addressType: 'PRINCIPAL' | 'COBRANCA', field: keyof EnderecoPayload, value: any) => {
    let newEnderecos = [...enderecos];
    let addressIndex = newEnderecos.findIndex(e => e.tipo_endereco === addressType);

    if (addressIndex === -1) {
      newEnderecos.push({ tipo_endereco: addressType, [field]: value });
    } else {
      newEnderecos[addressIndex] = { ...newEnderecos[addressIndex], [field]: value };
    }
  ...
  const handleCheckboxChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!isChecked) {
      onEnderecosChange(enderecos.filter(e => e.tipo_endereco !== 'COBRANCA'));
    } else {
      if (!enderecos.find(e => e.tipo_endereco === 'COBRANCA')) {
        onEnderecosChange([...enderecos, { tipo_endereco: 'COBRANCA' }]);
      }
    }
  };
    ...
    <Section title="Endereço" description="Endereço principal do parceiro e, opcionalmente, de cobrança.">
      <div className="sm:col-span-6 space-y-6">
        <AddressFields
          ...
          onAddressChange={(field, value) => handleAddressChange('PRINCIPAL', field, value)}
          ...
          {hasDifferentBillingAddress && (
            ...
            >
              <AddressFields
                ...
                onAddressChange={(field, value) => handleAddressChange('COBRANCA', field, value)}
